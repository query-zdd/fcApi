...
class showRegisterRoleView(APIView):
    ...
    def post(self, request):
        ...
        for done in data:
            d_num = d_num + 1
            valObj = RegisterRoleInsertSerializer(data=done)
            if not valObj.is_valid():
                d_flag = 1
                samp = {}
                samp['msg'] = valObj.errors
        ...
        if d_flag == 0:
            for done in data:
                ...
                try:
                    num = RoleMenu.objects.all().count()+1
                    mid = done["id"]
                    if mid:
                        bObj = RoleMenu.objects.get(id=mid)
                        bObj.update_time = dt
                    else:
                        bObj = RoleMenu()
                        bObj.create_time = dt
                    ...
                    bObj.archives_id = done['archives_id']
                    bObj.active = done['active']
                    bObj.password = done['password']
                    bObj.pc_client = done['pc_client']
                    bObj.app_client = done['app_client']
                    bObj.role_id = done['role_id']
                    if not mid:
                        bObj.weight = num
                    bObj.save()
                except:
    ...
    def delete(self, request):
        ...
        try:
            ...
            ids = data['ids']
            for one in ids:
                bObj = RoleMenu.objects.get(id=one)
                dt = datetime.now()
                bObj.delete_time = dt
                bObj.save()
            # 返回数据
...
class showRegisterRoleSortView(APIView):
    ...
    def put(self,request,bid):
        ...
        valObj = BasicSortSerializer(data=request.query_params)
        if valObj.is_valid():
            obj = RoleMenu.objects.all()
            offset = int(data['offset'])
            bid = bid
            begin_weight = RoleMenu.objects.get(id=bid).weight
            result = Msort(obj, offset, bid, begin_weight)
            if result:
                # 返回数据
                request = request.method + '  ' + request.get_full_path()
...
class showSociInsuranceView(APIView):
    #查询角色管理
    @csrf_exempt
    def get(self, request):
        ret, msg = checkPermission(request)
        ...
        try:
            bObj = SocialInsurance.objects.filter(delete_time=None).order_by("weight")
            temp = {}
            request = request.method + '  ' + request.get_full_path()
            temp["data"] = bObj.values()
            temp['message'] = "检索数据成功"
    ...
    def post(self, request):
        ...
        for done in data:
            d_num = d_num + 1
            valObj = showSociInsuranceSerializer(data=done)
            if not valObj.is_valid():
                d_flag = 1
                samp = {}
                samp['msg'] = valObj.errors
        ...
        if d_flag == 0:
            for done in data:
                ...
                try:
                    num = SocialInsurance.objects.all().count()+1
                    mid = done["id"]
                    if mid:
                        bObj = SocialInsurance.objects.get(id=mid)
                        bObj.update_time = dt
                    else:
                        bObj = SocialInsurance()
                        bObj.create_time = dt
                    bObj.social_insurance_level = done['social_insurance_level']
    ...
    def delete(self, request):
        try:
            ...
            ids = data['ids']
            for one in ids:
                bObj = SocialInsurance.objects.get(id=one)
                dt = datetime.now()
                bObj.delete_time = dt
                bObj.save()
            # 返回数据
...
class showSociInsuranceSortView(APIView):
    #基础资料排序
    def put(self,request,bid):
        data = request.query_params
        valObj = BasicSortSerializer(data=request.query_params)
        if valObj.is_valid():
            obj = SocialInsurance.objects.all()
            offset = int(data['offset'])
            bid = bid
            begin_weight = SocialInsurance.objects.get(id=bid).weight
            result = Msort(obj, offset, bid, begin_weight)
            if result:
                # 返回数据
                request = request.method + '  ' + request.get_full_path()
...
class showSurPluView(APIView):
    ...
    def get(self, request):
        ...
        try:
            bObj = Surplu.objects.filter(delete_time=None).order_by("weight")
            temp = {}
            request = request.method + '  ' + request.get_full_path()
            temp["data"] = bObj.values()
            temp['message'] = "检索数据成功"
    ...
    def post(self, request):
        ...
        for done in data:
            d_num = d_num + 1
            valObj = showSurpleSerializer(data=done)
            if not valObj.is_valid():
                d_flag = 1
                samp = {}
                samp['msg'] = valObj.errors
        ...
        if d_flag == 0:
            for done in data:
                ...
                try:
                    num = Surplu.objects.all().count()+1
                    mid = done["id"]
                    if mid:
                        bObj = Surplu.objects.get(id=mid)
                        bObj.update_time = dt
                    else:
                        bObj = Surplu()
                        bObj.create_time = dt
                    bObj.surplu_level = done['surplu_level']
    ...
    def delete(self, request):
        try:
            ...
            ids = data['ids']
            for one in ids:
                bObj = Surplu.objects.get(id=one)
                dt = datetime.now()
                bObj.delete_time = dt
                bObj.save()
            # 返回数据
...
class showSurPluSortView(APIView):
    #基础资料排序
    def put(self,request,bid):
        data = request.query_params
        valObj = BasicSortSerializer(data=request.query_params)
        if valObj.is_valid():
            obj = Surplu.objects.all()
            offset = int(data['offset'])
            bid = bid
            begin_weight = Surplu.objects.get(id=bid).weight
            result = Msort(obj, offset, bid, begin_weight)
            if result:
                # 返回数据
                request = request.method + '  ' + request.get_full_path()
